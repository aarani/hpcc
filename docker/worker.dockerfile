...
# The test on the gid is paranoia: if a future alpine release renumbers
...
RUN { getent group kvm >/dev/null || addgroup -S -g 36 kvm; } && \
    test "$(getent group kvm | cut -d: -f3)" = "36" && \
