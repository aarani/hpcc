...
| [Phase 5](plan/phase-5-observability.md) | Observability & Polish | In progress |
...
- [CAS-mode source dispatch](plan/cas.md) — the §4.5 content-addressed
  source-staging path. Shipped as the default `source_mode = "cas"`;
  PREPROCESSED stays selectable as a fallback. Probe-then-upload,
  `.hpcc` project marker for cross-developer hits, paranoid-mode-
  friendly trust boundary, `.S`/`.incbin` support.
